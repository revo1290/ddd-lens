...
const CONFIDENCE_COVERAGE = { high: 1, medium: 0.6, low: 0.25 };
function evaluateFit(fit) {
...
function mergeCriterion(config, automatic) {
    const definition = CRITERIA.find((item) => item.id === automatic.criterionId);
    if (!definition)
        throw new Error(`未知の評価項目です: ${automatic.criterionId}`);
...
function dimensionScores(criteria) {
...
function adoptionLabel(score, coverage) {
    if (score === null || coverage < 40)
        return "判定保留（根拠不足）";
    if (score < 25)
        return "名称先行または未適用";
    if (score < 50)
...
export function buildReport(config, detectors, stats, version = "0.1.0", now = new Date()) {
