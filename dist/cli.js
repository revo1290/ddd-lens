...
program.command("scan")
    ...
    if (!renderer)
        throw new Error(`未対応の形式です: ${options.format}`);
    const rendered = renderer();
    if (options.output)
        await writeFile(path.resolve(options.output), rendered, "utf8");
    else
        process.stdout.write(rendered);
    const ranks = { critical: 4, high: 3, medium: 2, low: 1, never: 99 };
    const threshold = ranks[options.failOn];
    if (threshold === undefined)
        throw new Error(`未対応のfail-onです: ${options.failOn}`);
...
program.command("explain")
    ...
    if (!definition)
        throw new Error(`未知の評価項目です: ${criterion}`);
