...
function assertScore(value, location) {
...
function validateConfig(input) {
    ...
    const raw = input;
    if (raw.version !== 1)
        throw new Error("version: 1 を指定してください");
    if (raw.fit && typeof raw.fit === "object") {
        for (const [key, value] of Object.entries(raw.fit)) {
            assertScore(value, `fit.${key}`);
        }
    }
    if (raw.assessment && typeof raw.assessment === "object") {
        for (const [id, value] of Object.entries(raw.assessment)) {
            if (!value || typeof value !== "object")
                throw new Error(`assessment.${id} はオブジェクトで指定してください`);
...
export async function loadConfig(root, explicitPath) {
    ...
    for (const candidate of candidates) {
        try {
            ...
            if (code === "ENOENT" && !explicitPath)
                continue;
            if (code === "ENOENT")
                throw new Error(`設定ファイルが見つかりません: ${candidate}`);
            throw error;
        }
    }
