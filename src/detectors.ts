import type { DetectorOutput, Evidence, Finding, ScanContext, ScannedFile } from "./types.js";

interface MatchSpec {
  pattern: RegExp;
  categories?: ScannedFile["category"][];
  pathPattern?: RegExp;
}

function locate(context: ScanContext, specs: MatchSpec[], max = 5): Evidence[] {
  const evidence: Evidence[] = [];
  for (const file of context.files) {
    for (const spec of specs) {
      if (spec.categories && !spec.categories.includes(file.category)) continue;
      if (spec.pathPattern && !spec.pathPattern.test(file.relativePath)) continue;
...
function docs(...patterns: RegExp[]): MatchSpec[] {
  return patterns.map((pattern) => ({ pattern, categories: ["document", "config"] }));
}

export function runDetectors(context: ScanContext): DetectorOutput[] {
  const results: DetectorOutput[] = [];

  results.push(positiveSignal(
    context,
    "strategy.subdomains",
    docs(/\bcore subdomain\b|コア[・ ]?サブドメイン/i, /supporting subdomain|支援[・ ]?サブドメイン/i, /generic subdomain|汎用[・ ]?サブドメイン/i),
    "サブドメイン分類を示す記述を検出しました",
    "サブドメインの事業上の分類はコードから判断できません"
  ));
  ...
  results.push(positiveSignal(
    ...
    "コンテキスト間関係を示す資料を検出しました",
    "コンテキスト間の関係と統合方針は自動確認できませんでした"
  ));
  ...
  results.push(positiveSignal(
    ...
    "用語集またはユビキタス言語の記述を検出しました",
    "共有されるドメイン用語の根拠を検出できませんでした"
  ));
  ...
  results.push(output(
    ...
    null,
    "low",
    "業務会話・文書・コードの意味的一致は静的な語句検索だけでは評価できません",
    []
  ));
  results.push(positiveSignal(
    context,
    "language.collaboration",
    docs(/event storming|eventstorming|modeling workshop|モデリング会|ドメインエキスパート|業務有識者/i),
    "協働モデリングの記録を検出しました",
    "継続的な協働モデリングはリポジトリだけでは判断できません"
  ));

  const domainFiles = context.files.filter((file) => file.category === "source" && /(?:^|\/)(?:domain|model)(?:\/|$)/i.test(file.relativePath));
  const domainBehaviorEvidence = locate(
    { ...context, files: domainFiles },
    [
  ...
  const anemicEvidence = locate(
    { ...context, files: domainFiles },
    [{ pattern: /\b(?:get|set)[A-Z]\w*\s*\(/ }],
    8
  );
  ...
  const frameworkEvidence = locate(
    { ...context, files: domainFiles },
    [{ pattern: /^\s*import\s+(?:org\.springframework|jakarta\.(?:persistence|ws|servlet)|javax\.(?:persistence|ws|servlet)|com\.fasterxml|express|next\/|@nestjs|sqlalchemy|django)/i }],
    8
  );
  if (domainFiles.length === 0) {
  ...
  results.push(positiveSignal(
    ...
    "アーキテクチャ制約の自動検証を検出しました",
    "境界をCIで検証する仕組みを検出できませんでした"
  ));
  ...
  results.push(positiveSignal(
    ...
    "外部モデルとの変換境界を示すシグナルを検出しました",
    "外部・他コンテキストとのモデル変換は自動確認できませんでした"
  ));
