import type { AssessmentReport, CriterionResult, Severity } from "./types.js";
...
function fitLabel(report: AssessmentReport): string {
  const labels = {
    recommended: "適用推奨",
    selective: "選択的適用",
...
function keyRisks(report: AssessmentReport, max = 5): string[] {
...
function criterionLine(criterion: CriterionResult): string {
...
export function renderText(report: AssessmentReport): string {
  const lines = [
...
export function renderJson(report: AssessmentReport): string {
